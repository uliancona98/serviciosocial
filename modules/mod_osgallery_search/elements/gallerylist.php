...
class JFormFieldSettingFromGallery extends JFormField{
  protected function getInput(){
    $db = JFactory::getDBO();
    $input = JFactory::getApplication()->input;
    $menuId = $input->get("id",0,"INT");
    ...
    if($view == 'item'){
        $selected = isset($params->setting_from_gallery)?$params->setting_from_gallery:'';
    }else{
        $selected = ($params->get('setting_from_gallery', '') != '')?$params->get('setting_from_gallery'):'';
    }
    $html = JHtmlSelect::genericlist($options, $this->name,
                'size="1" class="inputbox" ', 'value', 'text', $selected);
